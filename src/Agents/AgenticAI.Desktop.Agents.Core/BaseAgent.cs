...
public abstract class BaseAgent : IAgent, IDisposable
{
    ...
    protected readonly List<AgentCapability> _capabilitiesList;
    ...
    public IReadOnlyList<AgentCapability> Capabilities { get; protected set; }
    ...
    protected BaseAgent(ILogger logger, IConfiguration configuration, string name, string description)
    {
        ...
        _capabilitiesList = new List<AgentCapability>();
        ...
        Version = "1.0.0";
        Capabilities = _capabilitiesList.AsReadOnly();
    ...
    /// <summary>
    /// Execute a command (IAgent interface implementation)
    /// </summary>
    public async Task<AgentExecutionResult> ExecuteAsync(AgentCommand command, CancellationToken cancellationToken = default)
    {
    ...
    /// <summary>
    /// Shutdown the agent gracefully (IAgent interface implementation)
    /// </summary>
    ...
    /// <summary>
    /// Stop the agent (internal implementation)
    /// </summary>
    ...
    private async Task HandleCommandExecutionMessage(McpMessage message, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var commandData = message.Payload.ContainsKey("command") ? message.Payload["command"] : null;
            if (commandData != null)
            {
                var command = JsonSerializer.Deserialize<AgentCommand>(JsonSerializer.Serialize(commandData));
                if (command != null)
                {
